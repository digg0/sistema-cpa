...
export default function Resultados({ campanhas }: { campanhas: CampanhaApi[] }) {
  ...
  const [resultados, setResultados] = useState<ResultadosApi | null>(null)
  const [loading, setLoading] = useState(false)
  const [erro, setErro] = useState('')

  useEffect(() => {
    if (encerradas.length === 0) { setCampId(''); return }
    if (!encerradas.some(c => c.id === campId)) setCampId(encerradas[0].id)
    // Só reposiciona a seleção quando a campanha escolhida some da lista.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [encerradas])

  useEffect(() => {
    if (!campId) { setResultados(null); return }
    const controller = new AbortController()
    setLoading(true)
    setErro('')
    obterResultados(campId, controller.signal)
      .then(setResultados)
      .catch(error => {
        if (error instanceof DOMException && error.name === 'AbortError') return
        setErro(error instanceof ApiException ? error.message : 'Não foi possível carregar os resultados.')
      })
      .finally(() => setLoading(false))
    return () => controller.abort()
  }, [campId])

  const camp = encerradas.find(c => c.id === campId)
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <select value={campId} onChange={e => setCampId(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-700 min-w-[270px]">{encerradas.map(c => <option key={c.id} value={c.id}>{c.nome}</option>)}</select>
      </div>
      ...
      {loading && <Card className="p-14 text-center"><div className="mx-auto mb-3 h-8 w-8 rounded-full border-2 border-slate-200 border-t-green-700 animate-spin" /><p className="text-sm font-medium text-slate-500">Carregando resultados…</p></Card>}
      {!loading && erro && <Card className="p-8 text-center text-sm text-red-800 bg-red-50 border-red-200">{erro}</Card>}
